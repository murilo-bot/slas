...
function getSession(token, clientIP) {
    return invokeService(
        LocalServiceRegistry.createService('sfcc-slas-ocapi-session-bridge', {
            createRequest: function (svc) {
                svc.setRequestMethod('POST');
                svc.addHeader('Authorization', 'Bearer ' + token);
                svc.setURL(config.OCAPI_SESSION_BRIDGE_URI_SLAS);
            ...
            */
                if (clientIP && config.CLIENT_IP_HEADER_NAME) {
